...
function ProblemStatus() {
    ...
    const fetchData = async () => {
        try {
            ...
            const response = await axios.get("http://localhost:3001/getHostPendingProblems", {
                params: { NGOName }
            });
            console.log(NGOName);
    ...
    const completedProblem = async (problemId) => {
        ...
        const NGOname = cookie.get("NGOname");
        await axios({
            url: `http://localhost:3001/CompletedProblem?problemId=${problemId}&NGOname=${NGOname}`,
            method: "post"
        }).then((res => {
            if (res.data.success) {
                fetchData()
                ...
                <div style={{display : "flex", gap : "20px", alignItems : "center", justifyContent : "flex-start"}}>
                    {problems.length > 0 ? (
                        problems.map((problem, index) => (
                            <a href={"/problemdescription?problemid="+problem._id} style={{textDecoration : "none", color : "black"}} key={index} >
                                <div>
                                    ...
                                    <div style={{ display: "block" }}>
                                        ...
                                        <div className="ExProblemContainer" style={{marginTop : "50px"}}>
                                            ...
                                            <div style={{marginTop : "10px", margin : "0 auto", marginLeft : "20px"}}>
                                                <button className="CompletedButton" onClick={() => completedProblem(problem._id)} style={{border : "transparent", margin : "0 auto",marginTop : "10px"}}>
                                                        <div style={{ borderRadius : "10px", backgroundColor : "rgb(77, 130, 226)", color : "white",display : "flex", gap : "0px", alignItems : "center", justifyContent : "center", width : "150px"}}>
                                                            <div className="">
                                                                <img src={Thumbsup} alt="Tick" height="16px" width="16px" />
                                                            </div>
                                                            <div><p style={{marginLeft : "10px", marginRight : "10px", fontWeight : "bolder"}}>Completed</p></div>        
                                                        </div>
                                                </button>
                                            </div>
                                        </div>
